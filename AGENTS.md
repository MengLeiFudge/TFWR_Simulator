...
6. `BepInEx/LogOutput.log` 是第二优先级运行证据；模组应在请求运行中每秒输出 `item_snapshot`，包含当前所有物品数量，用于判断脚本是否仍在推进资源。
7. `state.json` 是最低优先级控制面；开始脚本前确认 `idle`，终止脚本后确认最终态即可，不得把状态机轮询当作脚本仍有进展的证据。
