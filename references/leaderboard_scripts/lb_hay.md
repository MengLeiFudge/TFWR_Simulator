...
- 2026-06-09 补水库存读取缓存候选：`water_pair_slot()` 原先低库存时会连续调用最多三次 `num_items(Items.Water)` 判断 `>2 / >1 / >0`；当前改为单次读取 `water` 后按原阈值分支。该候选不改变三桶 / 两桶 / 单桶 fallback、相邻双草 Bush-only 轮转、companion reroll 或目标检查。真实游戏当前仍为 `game_tick=0` timeout，暂未能跑完成轮；文件头成绩不更新。游戏恢复后与其他管理成本候选一起短跑，若无稳定刷新则回退。
