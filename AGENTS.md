...
1. `references/leaderboard_scripts/` 中的真实榜单脚本 `lb_*.py` 必须配套同名 `lb_*.md`；新增榜单脚本时也必须同步新增对应 md。
2. `references/leaderboard_scripts/README.md` 是全榜单共用注意事项主文档；后续维护榜单时，应先遵守该文件中的共用方法论，再看具体 `lb_xxx.md`。
3. `lb_*.py` 只允许保留：
   - 与实现强绑定的必要注释
   - 关键魔法数字来源说明
   - 当前脚本版本对应的时间 / 均值 / 证据摘要
4. `lb_*.py` 不应继续承载：
   ...
   - 通用方法论
   - 大段“当前建议 / 下一步方向”
5. 以上策略内容统一写入对应 `lb_*.md`，并按“已验证结论”与“猜测 / 待验证”的候选策略方向分层记录，不能混写。
6. `lb_start.py` 是部署时生成的入口，不属于具体榜单的长期策略真源，不需要也不应配套 `lb_start.md`。
