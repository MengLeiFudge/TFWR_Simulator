...
- 当前版本复跑时间：2026-06-08 请求 `633` 有效两轮 `4:34.453` / `4:34.363`，稳定均值 `4:34.408`；后续 `finished=false` 取消摘要不作为成绩。
...
- `lb_carrots()` 4x8 八锚点周期 tile
  ...
  - 结论：折线 8 锚点稳定刷新竖列 8 锚点 `4:41.588 -> 4:34.408`，保留为当前默认；收益来自减少 companion 落到锚点的 blocked 坐标，仍未解决静态单类型 support 的 `Grass / Tree` 利用率问题。
- `lb_carrots()` 4x8 十锚点 staircase
  ...
  - 结论：慢于当前折线 8 锚点 `4:34.408`，失败实现不保留在 `.py`。十锚点虽然增加锚点数量，但更长路径、更多跨列移动和更低单锚点 Bush 成功率抵消了理论收益；后续不要只按静态几何上限继续加密锚点。
- 折线 8 锚点瓶颈探针
...
- 动态 support 预算筛选
  ...
  - 安全的本 tile 内同无人机动态改写 distance<=3 估算 `5:32.201`，distance<=1 / <=2 更慢，均弱于当前折线 8 锚点 `4:34.408`。
