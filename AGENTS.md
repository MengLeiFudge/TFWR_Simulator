...
4. 评估外部候选或新策略时，`run_real_game_script.py` / `real_game_runner.py` 默认只跑到 `output.txt` 出现 2 条新增 `run=` 输出；如果两轮已经明显优于或劣于当前基线，应立刻停止，不要继续等更多轮。只有要刷完整官方统计或专门观察长尾均值时，才显式传 `--max-leaderboard-runs 0`。
