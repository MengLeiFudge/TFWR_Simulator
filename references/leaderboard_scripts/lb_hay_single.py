...
# Hay single 版本结论
# main1: 3min40s，3x3 递归补伴生，明显落后。
# main2: 3min37s，3x3 纯赌 Bush 伴生，明显落后。
# main3: 3min12s，3x3 十字补伴生，比 main1/main2 好，但仍不够快。
# main4: 手跑 2.52.217，5x5 双草轮转基线版本；结构简单，仍可继续作为参考。
# main5: 手跑 2.59.094，4x4 双草轮转，初始化更轻但 companion 命中率更差，放弃。
# main6: 手跑 2.52.693，main4 加日志；日志显示时间几乎完全受 refresh43/refresh44 总数控制。
# main7: 手跑 2.53.383，合法日志版；“慢局标记”可用于分析，但本身没有提速。
# main8: 手跑 2.53.633，补水后二次确认；late_wait 始终为 0，说明“补水后过早收获”不是问题。
# main9: 手跑 2.54.589，放行“另一块目标草位 + Grass companion”；other_grass43/44 始终为 0，假设错误。
# main10: 手跑 2.53.018，保留 main6 主策略并把两处双浇水合并为 use_item(Items.Water, 2)；日志样本平均值优于 main6，当前推荐入口。
# main11: 手跑约 3分32秒，动态改种位方案显著退化，放弃。
#
# 当前建议：
# 1. 默认从 main10 继续优化。
# 2. 后续优先围绕“如何减少 refresh43/refresh44 总数”继续做，而不是再碰已证伪方向。
# 3. 已证伪方向：4x4 双草轮转、补水后二次确认、另一块目标草位 + Grass companion、动态改种位。
#
