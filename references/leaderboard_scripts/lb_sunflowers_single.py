...
# Sunflowers single 版本结论
# main1: 5x5 满图向日葵 + 花瓣记录。
#        草稿里的单主位轮转没有必要，因为向日葵不走 companion；
#        这里直接维持 25 株常驻，并优先收当前最大花瓣拿倍率。
#
# 当前建议：
# 1. 默认从 main1 继续优化。
# 2. 每次重种后立刻 `measure()` 记花瓣，避免后续重复探测。
# 3. 主循环优先收最大花瓣；其余成熟向日葵先留在田里，保证场上始终有足够数量吃倍率。
