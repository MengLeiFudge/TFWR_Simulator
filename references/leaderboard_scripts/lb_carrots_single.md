...
- 当前可靠真实时间：请求 `400` 有效完成 `13` 轮，均值 `9:45.406`。
- 当前 `main4()` companion 路线：5x5 上只保留 `(1,1)/(3,1)/(1,3)/(3,3)` 四个胡萝卜锚点；其余格预置 `Bush` 支撑，初始化时记录每格实体，锚点只接受已知非锚点 `Bush` companion，成功命中后不再往返补支撑。
...
- `main4`
  ...
  - 结论：预置非锚点 Bush 支撑比命中后往返补支撑更快；记录已知 Bush 支撑后能略微减少确认成本，但仍慢于 #1 `3:46.963`
- 2026-04-30 已知支撑记录复测
