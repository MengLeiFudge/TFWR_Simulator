...
- 2026-06-08 探针后的收敛：
  ...
  - 2026-06-09 补水库存读取缓存候选：`water_carrot_anchor()` 原先低库存时会连续调用最多三次 `num_items(Items.Water)` 判断 `>384 / >192 / >64`；当前改为单次读取 `water` 后按原阈值分支。该候选不改变三桶 / 两桶 / 单桶阈值、补水量、折线 8 锚点、静态 Bush support、Bush-only companion 接受或目标 guard。真实游戏当前仍为 `game_tick=0` timeout，暂未能跑完成轮；文件头成绩不更新。游戏恢复后与统计删除候选一起短跑，若无稳定刷新则回退。
